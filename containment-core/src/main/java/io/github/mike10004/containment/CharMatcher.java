...
abstract class CharMatcher implements Predicate<Character> {
    ...
    private static final class ForPredicate extends CharMatcher {
        ...
        @Override
        public boolean test(Character character) {
            return predicate.test(requireNonNull(character));
        }
    ...
    private static void checkArgument(boolean mustBeTrue) {
        if (!mustBeTrue) {
            throw new IllegalArgumentException();
        }
    }

    private static void checkPositionIndex(int start, int length) {
